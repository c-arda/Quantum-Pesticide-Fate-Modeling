...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Microbial degradation proxies} (3): count of hydrolyzable
        ...
        to degrading organisms. Note that $B$ is derived from two
        \emph{independent} measurements (solubility and sorption), not from
        $K_\text{oc}$ alone, so it does not introduce circular prediction.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lllccc}
    ...
    \midrule
    \multirow{4}{*}{Ridge}
      & \degtl{} & LOO    & 0.154 \,[$-$0.075, 0.327] & 0.488 & 0.626 \\
      & \degtl{} & 5-fold & $-$0.109 $\pm$ 0.408     & 0.504 & 0.644 \\
      & \koc{}   & LOO    & 0.842 \,[0.751, 0.919]   & 0.252 & 0.413 \\
      & \koc{}   & 5-fold & 0.844 $\pm$ 0.063         & 0.248 & 0.405 \\
    \midrule
    \multirow{4}{*}{LASSO}
      & \degtl{} & LOO    & 0.101 \,[0.006, 0.164]   & 0.518 & 0.645 \\
      & \degtl{} & 5-fold & $-$0.050 $\pm$ 0.102     & 0.525 & 0.658 \\
      & \koc{}   & LOO    & 0.747 \,[0.556, 0.881]   & 0.336 & 0.522 \\
      & \koc{}   & 5-fold & 0.746 $\pm$ 0.138         & 0.341 & 0.529 \\
    \midrule
...
Key observations:
\begin{itemize}[nosep]
  \item \textbf{Linear models are competitive:} Ridge regression achieves
        the highest \koc{} \rtwo{} (0.842, LOO), surpassing both RF (0.766)
        and GB (0.779). This confirms that \koc{} prediction is
        well-described by a linear combination of the \nfeat{} descriptors,
        consistent with the dominance of log\,$P$ and bioaccessibility in
        \cref{fig:importance}.
  ...
  \item \textbf{DegT50 instability:} The fold-level standard deviations for
        ...
        exceed the means, confirming high prediction instability. Notably,
        RF improves from LOO to 5-fold for \degtl{} (lower MAE) while
        GB degrades, reflecting the sensitivity of \degtl{} predictions
        to fold composition rather than systematic model differences.
        In contrast, \koc{} fold variation ($\pm 0.06$--$0.14$) is modest.
\end{itemize}

Wilcoxon signed-rank tests on per-substance LOO squared residuals confirm
that none of the pairwise model differences are statistically significant
at $\alpha = 0.05$: RF vs.\ GB ($p = 0.19$ for \degtl{}, $p = 0.21$ for
\koc{}), Ridge vs.\ RF ($p = 0.16$, $p = 0.07$), and Ridge vs.\ GB
($p = 0.71$, $p = 0.18$). The near-significant Ridge vs.\ RF comparison
for \koc{} ($p = 0.07$) is consistent with the numerically large difference
in \rtwo{} (0.842 vs.\ 0.766) but does not reach conventional significance
with $N = \nsub{}$ substances.
...
\begin{figure}[htbp]
  ...
  \caption{Model comparison (5-fold CV, per-fold mean \rtwo{} $\pm$
           ...
           Ridge dominates \koc{} prediction, while all models perform
           poorly on \degtl{} with high fold-level variance.}
...
\begin{table}[htbp]
  ...
  \caption{Architecture experiment: DegT50 prediction (5-fold CV) across
           ...
           \rtwo{}~$= -0.141$ with 5 layers and 60 epochs.}
  \label{tab:arch}
  \small
  \begin{tabular}{llcccccc}
    ...
    \midrule
    \multicolumn{2}{l}{\emph{Current baseline (lr$=$0.05)}} & 12 & 5 & 60
      & --- & $-$0.141 & --- \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{enumerate}
  \item \textbf{A simple linear model dominates \koc{} prediction:}
        Ridge regression achieves the highest \koc{} \rtwo{} (0.842,
        \loocv{}), outperforming RF (0.766), GB (0.779), and the VQC
        (0.412). This implies that the quantum feature map provides no
        useful nonlinear structure beyond a linear combination of the
        \nfeat{} raw descriptors for sorption prediction.
