...
Random Forest (RF) achieves \rtwo{}~=~0.194 for \degtl{} and 0.726 for
\koc{} (on a circularity-free 16-feature set excluding the bioaccessibility
proxy), while Gradient Boosting (GB) reaches 0.223 and 0.764, respectively.
...
A hybrid stacking framework blending QML and RF predictions yields
$\alpha = 0.20$, but the resulting R\textsuperscript{2} improvement
(+0.02) is not statistically significant. We also assess adverse
...
below the GB 5-fold baseline of 0.740 (16-feature, circularity-free).

The full VQC \loocv{} (111~folds $\times$ 60~epochs) confirms these trends:
\rtwo{}~$= -0.035$ for \degtl{} and \rtwo{}~$= 0.533$ for \koc{}
(MAE~$= 0.571$ and $0.533$, RMSE~$= 0.692$ and $0.709$, respectively).
The \koc{} \loocv{} \rtwo{} is comparable to Ridge regression (0.546) but
substantially below RF (0.726) and GB (0.764) on the 16-feature set.
...
the QML and RF \loocv{} predictions. Of the 111~VQC folds, one substance
(Difenoconazole) appears twice in the checkpoint due to a resumption
overlap; after deduplication, 110 common substances are used for
blending. The optimized weight is $\alpha = 0.20$ for both properties,
yielding hybrid \loocv{} \rtwo{} of 0.213 for \degtl{} (+0.019 over RF
alone) and 0.783 for \koc{} (+0.017 over RF's 0.766 on the same 110
substances).
...
\begin{enumerate}
  ...
  \item \textbf{\degtl{} remains unpredictable from structure:} All
        models achieve \rtwo{}~$\leq 0.23$ (\loocv{}), and a systematic
  ...
  \item \textbf{Hybrid stacking yields marginal gains;} the optimized
        blending weight $\alpha = 0.20$ improves RF \rtwo{} by $\sim$0.02,
        but the improvement is not statistically significant and comes at
        $\sim$100$\times$ computational cost. AOP-derived MoA features
        provide no improvement, indicating that mechanism-of-action labels
        do not encode additional variance beyond structural descriptors.
