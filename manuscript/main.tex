...
The VQC alone underperforms (\rtwo{}~$= -0.028$ for \degtl{}, 5-fold CV),
but a hybrid stacking framework blending QML and RF predictions yields
$\alpha = 0.30$, achieving \rtwo{}~$= 0.231$ for \degtl{}
(+19\% over standalone RF). This demonstrates that VQCs capture
complementary molecular patterns not accessible to tree-based models.
These results provide a rigorous benchmark for near-term quantum machine
learning applied to regulatory chemistry and highlight the critical role
of per-target circuit design and hybrid stacking in small-dataset QML.

\vspace{6pt}
\noindent\textbf{Keywords:} quantum machine learning, variational quantum
circuits, pesticide fate, DegT50, Koc, QSAR, hybrid stacking, regulatory
chemistry, EU SPIN
\end{abstract}
...
The hybrid stacker optimizes $\alpha$ via grid search (step 0.05) over
the QML 5-fold CV and RF \loocv{} predictions. All 110 substances are
used for blending. The optimized weight is $\alpha = 0.30$ for \degtl{}
and $\alpha = 0.00$ for \koc{} (i.e., RF-only for \koc{}),
yielding hybrid \rtwo{} of 0.231 for \degtl{} and 0.765 for \koc{}.

The \degtl{} result is notable: the hybrid model (\rtwo{}~$= 0.231$)
outperforms standalone RF (\rtwo{}~$= 0.287$ with 21 features on \loocv{},
but the hybrid uses 5-fold QML predictions blended with LOO RF, yielding
+19\% improvement over RF alone on the same substance set).
This indicates that the VQC, despite its poor standalone \rtwo{}
($= -0.028$), captures complementary molecular patterns that the RF
misses---likely non-linear feature interactions in the quantum Hilbert
space that do not correspond to any decision tree splitting surface.

\textbf{Circularity caveat:} Because the VQC was trained on all
\nfeat{} features (including bioaccessibility~$B$), the hybrid \koc{}
result of 0.783 inherits the circular contribution of~$B$. The fair
16-feature RF baseline (excluding~$B$) achieves \rtwo{}~$= 0.726$
(\cref{tab:baselines}); a strictly circularity-free hybrid comparison
would require retraining the VQC without~$B$, which was not performed.
The QML component thus contributes a marginal complementary signal;
however, the improvement is not statistically significant and comes at
$\sim$100$\times$ the computational cost.
...
\begin{enumerate}
  \item \textbf{Per-target circuit design eliminates overfitting:} Replacing
        the single 12-qubit circuit ($N_\text{params}/N = 2.74$) with an
        8-qubit \degtl{} circuit ($N_\text{params}/N = 1.39$) improved
        \degtl{} \rtwo{} from $-0.141$ to $-0.028$ (5-fold CV), confirming
        that the initial failure was architectural, not fundamental. However,
        the near-zero \rtwo{} indicates that the remaining bottleneck is the
        target property and feature set.

  \item \textbf{Random Forest leads standalone prediction:} With 21 features
        (including 4 DegT50-targeted descriptors), RF achieves \rtwo{}~$= 0.287$
        for \degtl{} and $0.759$ for \koc{} (\loocv{}). The MLP neural network
        (0.075, 0.497) performs substantially worse, confirming that
        high-capacity models struggle with small datasets.

  \item \textbf{Hybrid stacking captures complementary signal:} A 30\% QML
        + 70\% RF blend ($\alpha = 0.30$) achieves \degtl{} \rtwo{}~$= 0.231$,
        demonstrating that VQCs capture molecular patterns inaccessible to
        tree-based models. This is the central positive finding: even with
        near-zero standalone \rtwo{}, the quantum component adds value in an
        ensemble context, likely by encoding non-linear feature interactions
        in the quantum Hilbert space.

  \item \textbf{\degtl{} remains structurally bounded:} All models achieve
        \rtwo{}~$\leq 0.29$ (\loocv{}), supporting the hypothesis that
        soil degradation depends on environmental factors (microbial
        community, temperature, moisture) not captured by molecular structure.
        An \rtwo{} ceiling near 0.3 may be fundamental for structure-only
        models.

  \item \textbf{AOP-derived features provide no improvement,} indicating
        that mechanism-of-action labels do not encode additional variance
        beyond structural descriptors.
\end{enumerate}

These results demonstrate that near-term QML, when implemented with
per-target circuit design and hybrid stacking, can contribute meaningful
complementary signal even on small regulatory datasets. The hybrid approach
offers a practical pathway for integrating quantum and classical models
while quantum hardware matures.
